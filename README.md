...
기능적 요구사항
1.	관리자는 회의실을 등록할 수 있다.
2.	고객이 회의실를 선택해 예약하면 결제가 진행된다.
3.	예약이 결제되면 회의실의 예약 가능 여부가 변경된다.
4.	회의실이 예약 불가 상태로 변경되면 예약이 확정된다.
5.	고객은 예약을 취소할 수 있다.
6.	예약이 취소되면 결제가 취소되고 회의실의 예약 가능 여부가 변경된다.
7.	고객은 회의실 예약가능여부를 확인할 수 있다.

비기능적 요구사항
1. 트랜잭션
    1. 회의실 예약은 결제가 취소된 경우 불가해야한다.  Sync 호출
